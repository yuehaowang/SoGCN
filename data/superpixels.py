...
def to_undirected(g):
    ...
    has_edges = g.has_edges_between(dst, src).to(dtype=torch.bool)
    g.add_edges(dst[~has_edges], src[~has_edges])
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def _to_undirected(self):
        self.train.graph_lists = [to_undirected(g) for g in self.train.graph_lists]
        self.val.graph_lists = [to_undirected(g) for g in self.val.graph_lists]
        self.test.graph_lists = [to_undirected(g) for g in self.test.graph_lists]
